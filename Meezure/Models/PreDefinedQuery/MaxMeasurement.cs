...
namespace Meezure
{
	public class MaxMeasurement : BasePredefinedQuery<MeasurementInstanceModel> 
	{
		class GroupValue {
		...
		{
			string selectStr = "SELECT t1.MeasurementGroupDefinitionId, Max(t1.Value) AS Value FROM MeasurementGroupInstanceModel AS t1 JOIN MeasurementInstanceModel AS t2 ON t1.MeasurementInstanceId == t2.Id  WHERE t2.MeasurementSubjectId == ? AND t2.MeasurementDefinitionId == ? GROUP BY t1.MeasurementGroupDefinitionId";
		...
		public MaxMeasurement (IRepository<MeasurementInstanceModel> repository, IRepository<MeasurementGroupInstanceModel> groupRepository)
		{
